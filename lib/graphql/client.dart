...
import 'package:sliding_navbar/graphql/querys.dart';
...
class ApiService{
  ...
  Future<HomeState> getBanners() async {
    ...
    if (result.hasException) {
      ...
      return ErrorLoad(error: result.exception?.raw.toString()??"Error loaded");// uzingiz hohlagan erroringizni yozib qo`yishingiz mumkin
    } else {
      ...
      if (data != null && data['Page'] != null) {
        ...
        final List<BannerMedia> banners = List<BannerMedia>.from(mediaList.map((item) => BannerMedia.fromJson(item)));
